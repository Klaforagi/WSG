...
-- Purchase flow lives in Shop > Boosts.
-- Activation flow lives in Inventory > Boosts.
--------------------------------------------------------------------------------
...
local _UITheme
pcall(function()
    _UITheme = require(script.Parent.UITheme)
end)
...
function BoostsUI.Create(parent, _coinApi, _inventoryApi)
    ...
    local BoostConfig = safeRequireBoostConfig()
    ...
    helper.Text = "This screen is informational; buy in Shop and activate in Inventory."
    ...
    for index, def in ipairs(boostDefs) do
        ...
        card.Name = "Boost_" .. tostring(def.Id)
        ...
        title.Text = tostring(def.DisplayName or def.Id or "Boost")
        ...
        desc.Text = tostring(def.Description or "")
