...
applyQuestList = function(result)
	...
	local needsRebuild = questIndexesChanged(newQuests)

	latestQuests = newQuests
	questPositionByIndex = newPositionByIndex
	...
	if needsRebuild then
		...
		for _, quest in ipairs(latestQuests) do
			local row = rowByQuestIndex[quest.index]
