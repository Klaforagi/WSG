...
local function getMaxEquippedPotions()
	if type(PotionConfig.GetMaxEquippedPotions) == "function" then
		return PotionConfig.GetMaxEquippedPotions()
	end
	return 3
end
...
local function sanitizeEquippedPotions(state)
	local rawIds = {}
	if type(state.equippedPotionIds) == "table" then
		for _, potionId in ipairs(state.equippedPotionIds) do
			table.insert(rawIds, potionId)
		end
	elseif type(state.equippedPotionId) == "string" and state.equippedPotionId ~= "" then
		table.insert(rawIds, state.equippedPotionId)
	end

	local compact = {}
	local seen = {}
	local maxEquipped = getMaxEquippedPotions()
	for _, potionId in ipairs(rawIds) do
		if type(potionId) == "string" and potionId ~= "" and not seen[potionId] and getPotionDefinition(potionId) then
			local entry = state.potions and state.potions[potionId]
			local count = math.max(0, math.floor(tonumber(type(entry) == "table" and entry.count or 0) or 0))
			if count > 0 then
				seen[potionId] = true
				table.insert(compact, potionId)
				if #compact >= maxEquipped then
					break
				end
			end
		end
	...
	state.equippedPotionIds = compact
	state.equippedPotionId = compact[1]
	return compact
end

local function sanitizeEquippedPotion(state)
	local compact = sanitizeEquippedPotions(state)
	return compact[1]
end

local function isPotionInEquippedList(state, potionId)
	if type(potionId) ~= "string" or potionId == "" then
		return false
	end
	for _, equippedId in ipairs(sanitizeEquippedPotions(state)) do
...
local function removeEquippedPotion(state, potionId)
	local compact = {}
	for _, equippedId in ipairs(sanitizeEquippedPotions(state)) do
		if equippedId ~= potionId then
			table.insert(compact, equippedId)
		end
	end
	state.equippedPotionIds = compact
	state.equippedPotionId = compact[1]
...
local function snapshotCombatModifiers(player)
	local damage = {}
	local flat = {}
	local now = getServerTime()

	local damageMods = activeOutgoingDamageModifiers[player]
	if damageMods then
		for modifierId, modifier in pairs(damageMods) do
			local remaining = math.max(0, (tonumber(modifier.expiresAt) or 0) - now)
			if remaining > 0 then
				damage[modifierId] = {
					multiplier = tonumber(modifier.multiplier) or 1,
					remaining = remaining,
	...
	local flatMods = activeOutgoingFlatModifiers[player]
	if flatMods then
...
local function normalizeState(raw)
	...
	if type(raw.potions) == "table" then
		...
		if type(raw.equippedPotionIds) == "table" then
			state.equippedPotionIds = raw.equippedPotionIds
		else
	...
	if type(raw.combatModifiers) == "table" then
		state.combatModifiers = {
			damage = type(raw.combatModifiers.damage) == "table" and raw.combatModifiers.damage or {},
			flat = type(raw.combatModifiers.flat) == "table" and raw.combatModifiers.flat or {},
		}
	end

	sanitizeEquippedPotions(state)
	return state
end

local function getSavedState(player, state)
	...
	local equippedPotionIds = sanitizeEquippedPotions(normalized)
	return {
		potions = potions,
		equippedPotionIds = equippedPotionIds,
		equippedPotionId = equippedPotionIds[1],
		combatModifiers = snapshotCombatModifiers(player),
	}
end
...
local function buildPotionSnapshot(state, potionId)
	...
	return {
		...
		equipped = isPotionInEquippedList(state, potionId) and count > 0,
	}
end

local function getStateSnapshot(player, state)
	local pd = state or ensurePlayerData(player)
	local equippedPotionIds = sanitizeEquippedPotions(pd)
	local equippedPotionId = equippedPotionIds[1]
...
local function scheduleOutgoingDamageExpiry(player, modifierId, token, durationSeconds)
	task.delay(durationSeconds, function()
		...
		effectStartedEvent:Fire(player, {
			potionId = modifierId,
			duration = 0,
			expiresAt = getServerTime(),
		})
		markDirty(player, "damage_mod_expired", { force = true })
	end)
end
...
local function scheduleOutgoingFlatExpiry(player, modifierId, token, durationSeconds)
	task.delay(durationSeconds, function()
		...
		markDirty(player, "flat_mod_expired", { force = true })
	end)
end
...
local function applyPotionEffect(player, potionDef, humanoid)
	...
	if potionDef.EffectType == OUTGOING_DAMAGE_EFFECT then
		...
		markDirty(player, "damage_mod_applied", { force = true })
...
local function restoreCombatModifiers(player, rawModifiers)
	if type(rawModifiers) ~= "table" then
		return
	end

	if type(rawModifiers.damage) == "table" then
		for modifierId, entry in pairs(rawModifiers.damage) do
			local remaining = math.max(0, math.floor(tonumber(entry.remaining) or 0))
			local multiplier = tonumber(entry.multiplier) or 1
			if remaining > 0 and multiplier > 0 then
				local modifiers = activeOutgoingDamageModifiers[player]
				if not modifiers then
					modifiers = {}
					activeOutgoingDamageModifiers[player] = modifiers
				...
				modifiers[modifierId] = {
					...
					expiresAt = getServerTime() + remaining,
					token = token,
					source = entry.source,
				}
				scheduleOutgoingDamageExpiry(player, modifierId, token, remaining)
				effectStartedEvent:Fire(player, {
					potionId = modifierId,
					duration = remaining,
					expiresAt = getServerTime() + remaining,
...
function HealthPotionService:LoadProfileForPlayer(player)
	...
	if loadedPlayers[player] == true and playerData[player] then
		return {
			status = "existing",
			data = DataStoreOps.DeepCopy(getSavedState(player, playerData[player])),
		}
	end
	...
	restoreCombatModifiers(player, playerData[player].combatModifiers)
...
function HealthPotionService:SaveProfileForPlayer(player, currentData)
	...
	local payload = getSavedState(player, pd)
	local key = getKey(player)
...
function HealthPotionService:ClearTemporaryPotionEffects(player)
	...
	local damageMods = activeOutgoingDamageModifiers[player]
	if damageMods then
		for modifierId in pairs(damageMods) do
			clearedPotionIds[modifierId] = true
		end
	end
	local flatMods = activeOutgoingFlatModifiers[player]
	if flatMods then
		for modifierId in pairs(flatMods) do
			clearedPotionIds[modifierId] = true
		end
	end
	...
	markDirty(player, "temp_effects_cleared", { force = true })
	return next(clearedPotionIds) ~= nil
end
...
function HealthPotionService:SetOutgoingFlatModifier(player, modifierId, flatAdd, durationSeconds, source)
	...
	-- duration 0 means the elixir is done. Persist that.
	if durationSeconds <= 0 then
		...
		markDirty(player, "flat_mod_cleared", { force = true })
		return true
	end
	...
	markDirty(player, "flat_mod_applied", { force = true })
	effectStartedEvent:Fire(player, {
		potionId = modifierId,
		duration = durationSeconds,
		expiresAt = expiresAt,
...
function HealthPotionService:SetEquipped(player, shouldEquip, potionId)
	...
	if shouldEquip then
		...
		local equippedPotionIds = sanitizeEquippedPotions(pd)
		if isPotionInEquippedList(pd, resolvedPotionId) then
			return true, string.format("%s equipped", potionDef.DisplayName), self:GetState(player)
		end
		if #equippedPotionIds >= getMaxEquippedPotions() then
			return false, "All potion slots are full", self:GetState(player)
		end

		table.insert(pd.equippedPotionIds, resolvedPotionId)
		sanitizeEquippedPotions(pd)
		markDirty(player, resolvedPotionId .. "_equip", { force = true })
		fireStateChanged(player)
		return true, string.format("%s equipped", potionDef.DisplayName), self:GetState(player)
	end

	local resolvedUnequipId = type(potionId) == "string" and potionId or nil
	local equippedPotionIds = sanitizeEquippedPotions(pd)
	if resolvedUnequipId then
		if not isPotionInEquippedList(pd, resolvedUnequipId) then
			return true, "Potion unequipped", self:GetState(player)
		end
		removeEquippedPotion(pd, resolvedUnequipId)
	else
		if #equippedPotionIds == 0 then
			return true, "Potion unequipped", self:GetState(player)
		end
		pd.equippedPotionIds = {}
...
function HealthPotionService:UseEquippedPotion(player, potionId)
	if not player then
	...
	local equippedPotionIds = sanitizeEquippedPotions(pd)
	local equippedPotionId = type(potionId) == "string" and potionId or equippedPotionIds[1]

	if not equippedPotionId then
		return false, "Equip a potion first", { state = currentState }
	end
	if not isPotionInEquippedList(pd, equippedPotionId) then
		return false, "Equip a potion first", { state = currentState }
	end

	local potionDef = getPotionDefinition(equippedPotionId)
	if not potionDef then
		removeEquippedPotion(pd, equippedPotionId)
		fireStateChanged(player)
